# Page: Enquire

| Field | Value |
|---|---|
| **URL** | `https://jungleejourneys.com/enquire/` |
| **Focus keyword** | book tiger safari |
| **Meta title** (60 char) | Book Tiger Safari India \| Plan Your Journey \| Junglee Journeys |
| **Meta description** (149 char) | Start planning your tiger safari in India. Share your travel dates and preferences. Receive a personalized itinerary within 24 hours. No obligation. |
| **H1** | Plan Your Safari |
| **Search intent** | Transactional (63%) |
| **Content type** | Conversion form |
| **Positioning angle** | Angle 5 (Sighting Specificity) for the trust strip; Angle 4 (Boring Middle) for the "what happens next" reassurance. The form itself runs in professional register. |
| **Bolded keywords** | book tiger safari, tiger safari booking, safari booking india, tiger reserve booking, plan tiger safari, safari enquiry india |

---

## Hero

# **Plan Your Safari**

The form below takes about three minutes. We use it to figure out which park, which dates, and which lodge tier are right for the trip you have in mind. Within 24 hours of you submitting it, we send back a written quote with an itemised itinerary. There is no deposit until you say yes, and there is no obligation to say yes at all.

If you are not sure what you want yet, fill in the form anyway and tell us in the message field. The questions we ask once we receive an enquiry are usually the questions that help guests figure out what they actually want.

---

## Tell Us About Your Trip

### Your details

| Field | Required |
|---|---|
| Full name | Yes |
| Email | Yes |
| Phone or WhatsApp (with country code) | Yes |
| Country | Yes |

### Trip details

| Field | Required | Notes |
|---|---|---|
| Travel dates | No | Approximate dates are fine. "Sometime in March 2026" works. |
| Duration | Yes | Weekend (2 to 3 nights), Short break (4 to 5 nights), Week+ (7 nights or more), Not sure yet |
| Number of travellers | Yes | Adults and children separately |
| Preferred destinations | No | Multi-select. Kanha, Tadoba, Ranthambore, Bandhavgarh, Pench, Satpura, Corbett, Kaziranga, Panna, Manas, Gir, or "Not sure, recommend something" |

### What you are after

| Field | Required | Notes |
|---|---|---|
| What interests you most? | No | Tigers, photography, luxury lodges, off-beaten experiences, family-friendly, first tiger safari, multi-park itineraries |
| Accommodation preference | No | Luxury, mid-range, best value, no preference |
| Anything else we should know? | No | Special requests, dietary needs, accessibility, things you definitely do not want |

**[Send Enquiry]**

---

## What Happens Next

The **tiger safari booking** process with us is built around a written quote you can actually compare against other operators, not a phone call you have to take a decision on.

### 1. We read your enquiry

Usually within a few hours of you sending it, always within 24. If anything is unclear, we send a quick reply to ask, rather than guessing.

### 2. We send you a written quote

Itemised. Lodge by lodge, drive by drive, park by park, transfer by transfer. No hidden fees and no asterisks. The quote is detailed enough that you can compare it line by line against any other quote you have received.

### 3. You ask for changes

This is the most common step, and it is the part of **safari booking india** that most operators skip. You will probably want to swap a lodge, add a day, change a park, or move the dates. We expect this. We will revise the quote until it actually fits the trip you want, and we will keep revising it until you are sure.

### 4. You confirm and we lock the trip

Once you say yes, we lock the permits and the lodge rooms. You pay a 30% deposit. The final balance is due 30 days before departure. From that point on, the trip is fixed and the only thing left for you to do is turn up at the airport.

There is no deposit at any point before you say yes. You can ask as many questions as you want without commitment. The team that handles the enquiry is the team that runs the trip, so you are not getting passed between sales and operations.

---

## Why Book a Tiger Safari with Junglee Journeys

The numbers we are willing to put on the front page:

- **15+ years** running tiger safaris across India
- **500+** trips planned
- **70%** of our guests come back for a second trip
- **95%** of our guests see a tiger at least once on their trip
- **24 hours** turnaround on the first written quote

When you decide to **book tiger safari** dates with us, you are paying for a guide who has tracked the local tigress since she was a cub, a lodge we have personally slept in, and the entire boring middle of the trip handled by people who do this every week. You are also paying for a quote that arrives quickly, reads honestly, and does not change after you have signed it.

If your dates are running tight on permits in the high-demand months (March to May at Ranthambore in particular), we can hold the slots informally for 48 hours while you decide.

---

## Prefer to Talk?

If you would rather skip the form and just message a human, all three direct channels are open.

- **WhatsApp:** [+91 XXXXX XXXXX](https://wa.me/91XXXXXXXXXX) (fastest, even for international guests)
- **Phone:** [+91 XXXXX XXXXX](tel:+91XXXXXXXXXX) (Mon to Sat, 10 AM to 7 PM IST)
- **Email:** [hello@jungleejourneys.com](mailto:hello@jungleejourneys.com)

A WhatsApp message is faster than the form for **plan tiger safari** questions when your dates are flexible and you just want a rough quote. The form is faster when your dates are fixed and you want a complete itinerary on the first reply.

---

## A Few Common Questions

### Is there a deposit at the enquiry stage?

No. There is no payment of any kind until you have confirmed an itinerary in writing. The quote, the revisions, and the planning conversation are all free.

### How quickly will I hear back?

Within 24 hours, almost always faster. If you submit on a weekend or a holiday, the first reply might come on the next working day.

### Can I send a **safari enquiry india** without committing to dates?

Yes. About a third of our enquiries arrive without firm dates. We send back rough pricing for two or three likely scenarios and then narrow down once you decide.

### Do you handle **tiger reserve booking** as a standalone service, separate from the lodge?

Not usually. The permits, the lodge, and the guide are tightly linked, and separating them tends to produce worse trips at higher prices. We bundle everything because that is how the trip actually works on the ground.

---

## Ready

Fill in the form, message us on WhatsApp, or call. Whichever is easier for you. Same team, same 24-hour response time.

**[Send Enquiry](#enquiry-form)**   **[WhatsApp Us](https://wa.me/91XXXXXXXXXX)**

---

## SEO Check

| Item | Status |
|---|---|
| Focus keyword "book tiger safari" in body (target 2-3) | ✓ "Why Book a Tiger Safari" H2 (1), "book tiger safari dates" in body (1). Total: 2 occurrences. |
| Focus keyword stuffing test | ✓ Comfortable. |
| Meta title 60 chars, includes focus keyword + brand | ✓ |
| Meta description 149 chars | ✓ |
| H1 matches CSV | ✓ "Plan Your Safari" |
| Secondary keywords present | ✓ tiger safari booking (What Happens Next intro), safari booking india (step 3), tiger reserve booking (FAQ) |
| Tertiary keywords present | ✓ plan tiger safari (Prefer to Talk section), safari enquiry india (FAQ) |
| Internal links to /contact/, /about/ | Both implicitly via header/footer; explicit /contact info in Prefer to Talk section duplicates Contact page details |
| Em dashes | None |
| Staccato fragment openers | None |
| Phone, WhatsApp, email | All placeholders for client |
| Word count | ~900 |
