...
**Auth (since June 2026):** wrangler uses a non-expiring API token, not OAuth. `CLOUDFLARE_API_TOKEN` (scoped to Account → Cloudflare Pages → Edit) and `CLOUDFLARE_ACCOUNT_ID` are set as Windows user environment variables on this machine — deploys never prompt for login. If a deploy fails with auth error 10000, the token was likely revoked; create a new one in the Cloudflare dashboard (My Profile → API Tokens) and update the env var. Note: a long-running/automation shell caches its environment at launch, so a User-scoped var set in a *previous* session may not be visible — pull it in explicitly before deploying: `$env:CLOUDFLARE_API_TOKEN = [Environment]::GetEnvironmentVariable("CLOUDFLARE_API_TOKEN","User"); $env:CLOUDFLARE_ACCOUNT_ID = [Environment]::GetEnvironmentVariable("CLOUDFLARE_ACCOUNT_ID","User")`. Do not use `wrangler login` (OAuth) — it requires the account owner's credentials. For dashboard work (Redirect Rules, env vars, custom domains), `rankwithsourabh@gmail.com` is a member of the Cloudflare account (added June 2026).
...
│       ├── headings.ts                 slugifyHeading (pure, no fs — safe for client + server imports)
│       └── content.ts                  DESTINATIONS + SAFARIS arrays, markdown readers, + section/FAQ parsers
├── content/
...
| `NumberCounter` | Animated count-up for hero stats. Renders 0 until its IntersectionObserver fires, so it shows 0 in flat full-page screenshots — that is expected, it animates on real scroll. |
...
- **Detail-page structure (June 2026):** `destination/[slug]/page.tsx` no longer dumps one `MarkdownContent`. It calls `getSectionHeadings` + `extractFaqSection` (both in `content.ts`) and renders: `Breadcrumbs → [sticky PageTOC | MarkdownContent(before) + FAQAccordion(items) + MarkdownContent(after)] → CurrencyConverter → RelatedParks`, plus `TouristAttraction` and `FaqJsonLd` JSON-LD. This is a **template-level** upgrade, so every destination page automatically gets the TOC, anchored headings, FAQ accordion, and FAQ schema with no per-file edits. `extractFaqSection` finds the `## … FAQs` H2, parses `### question` + answer pairs, and splits the body around it so the trailing "Ready for…" section still renders in order. **The safari detail template (`safari/[slug]`) has NOT been migrated yet** — it still uses the old single-`MarkdownContent` render; apply the same wiring during rollout.
- **"Safe tables" content convention (June 2026):** scannable summary tables in the destination markdown (e.g. Kanha's Best Time / How to Reach / Where to Stay) are added in plain markdown (MarkdownContent already styles tables). Rule: tables only re-present facts already written on the page (no invented sighting %, prices, or logistics — see global no-fabrication / no-pricing / no-em-dash rules), and keyword-bearing prose is preserved. Only `01-kanha.md` has these so far (pilot); the other 10 destinations + 3 safaris are the rollout.
- **About page (`about/page.tsx`):** rebuilt from a plain `MarkdownContent` dump into a structured layout — founder-story prose still sourced from `02-about.md` (sliced at `## The Team`), but Team renders as cards and The Numbers as a `NumberCounter` stat band, both from typed arrays in the page file (kept in sync with the markdown manually; the markdown's Team/Numbers/CTA sections are no longer rendered).
